...
>
> **PHASE 2 (Task 6) — ABSORBED 2026-07-10 by the preprocessing split** (per-sample `pathway_score.R` deleted; the aggregate merge input was re-pointed `scored.rds`→`norm.rds`, one stage earlier than the `typed.rds` this plan assumed — see `plan-preprocessing.md`). Historical note follows: originally** — gated on the aggregate rebuild (`plan-aggregate-refactor.md`): the `scored.rds`→`typed.rds` merge-input change must run *with* that rebuild, not piecemeal against the locked aggregate. **NOTE 2026-06-22:** the aggregate rebuild (`plan-aggregate-refactor.md` Steps 4–5) ran and reproduced labels bit-identically, but it **kept the `scored.rds` merge input** — Phase 2 (scored→typed rename + delete `pathway_score.R`) was NOT folded in and remains pending for a future deliberate merge re-run. Code edits are stageable anytime; the run waits so the merge re-runs once. The plan's live numeric reproduction (execute `gsea.R`/`pathway_summary.R`, diff result tables) also rides that rebuild — Phase 1 validated only static gene-membership equivalence.
